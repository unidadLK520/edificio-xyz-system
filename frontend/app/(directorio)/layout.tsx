      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 md:w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col h-full transition-transform duration-300 ease-in-out transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:flex`}>
        ...
        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
          <div>
            ...
            <span className="text-xs text-slate-500 uppercase font-semibold tracking-wider block mt-1">Panel Directorio</span>
          </div>
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
              <a href="/reportes" onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 px-4 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white rounded-xl transition-colors">Reportes Mensuales</a>
              <a href="/auditoria" onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 px-4 text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white rounded-xl transition-colors">Auditoría</a>
